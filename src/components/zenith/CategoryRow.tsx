...
export const CategoryRow: React.FC<CategoryRowProps> = ({
  ...
  if (loading) {
    return (
      <section className="mt-6 px-4">
        ...
        <div className={`flex gap-4 ${layout === 'scroll' ? 'overflow-hidden' : 'grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5'}`}>
           {[...Array(5)].map((_, i) => (
               <div key={i} className="w-full aspect-[3/4] bg-white/5 rounded-2xl animate-pulse shrink-0" />
           ))}
        </div>
      </section>
    );
  ...
  return (
    <section className="mt-6 relative group">
        <SectionHeader title={title} icon={icon} actionText="See All" actionLink={viewAllLink} />
        ...
        <div 
            ...
            className={
              layout === 'scroll' 
                ? "flex gap-4 overflow-x-auto hide-scrollbar px-4 pb-4 snap-x snap-mandatory md:gap-6"
                : "grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 px-4 pb-4 md:gap-6"
            }
        >
            {manhwaList.map((manga, i) => (
                <div key={manga.id} className={layout === 'scroll' ? "snap-center flex-none" : ""}>
                  <MangaCard 
                      manhwa={manga} 
