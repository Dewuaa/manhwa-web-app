...
import { Search, Crown, Star, ChevronRight, Swords, Sparkles, Heart, Flame, Cpu, Ghost } from 'lucide-react';
...
export const Discover: React.FC<DiscoverProps> = ({ topManhwa = [], loading }) => {
  const genres = [
    ...
    { name: 'Fantasy', color: 'from-blue-500 to-cyan-500', icon: Sparkles, slug: 'fantasy' },
    { name: 'Romance', color: 'from-pink-500 to-rose-500', icon: Heart, slug: 'romance' },
    { name: 'Murim', color: 'from-emerald-500 to-green-500', icon: Flame, slug: 'martial-arts' },
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-10">
        ...
        <section>
            <div className="flex justify-between items-end mb-5">
                ...
                <Link href="/genres/all" className="text-xs text-blue-400 font-bold flex items-center gap-1 hover:underline">
                      ...
                      >
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center p-2">
                              <IconComponent 
                                ...
                                strokeWidth={2.5}
                              />
                              <span className="text-white font-bold text-sm drop-shadow-md tracking-wide">{genre.name}</span>
                          </div>
                      </Link>
                    );
                })}
                    ...
                    {topManhwa.slice(0, 9).map((manga, index) => (
                        ...
                        >
                            <div className="relative w-20 h-28 shrink-0 rounded-xl overflow-hidden shadow-lg">
                                ...
                                />
                                <div className={`absolute top-0 left-0 w-6 h-6 flex items-center justify-center rounded-br-lg font-black text-xs ${
                                    index === 0 ? 'bg-amber-500 text-black' : 
                                    index === 1 ? 'bg-gray-300 text-black' :
                                    index === 2 ? 'bg-amber-700 text-white' : 'bg-black/60 text-white backdrop-blur-sm'
                                }`}>
                                    {index + 1}
                                </div>
                            </div>
                            <div className="flex-1 py-1 flex flex-col min-w-0">
                                <h3 className="font-bold text-white text-base line-clamp-2 group-hover:text-blue-400 transition-colors mb-1">{manga.title}</h3>
                                <p className="text-gray-400 text-xs mb-2 truncate">{manga.latestChapter || 'Latest chapter'}</p>
                                <div className="mt-auto flex items-center gap-3 text-xs font-medium text-gray-500">
                                    ...
                                    {manga.rating && manga.status && manga.status !== 'Unknown' && <span>•</span>}
                                    {manga.status && manga.status !== 'Unknown' && <span>{manga.status}</span>}
                                </div>
                            </div>
                        </Link>
                    ))}
            ...
            <div className="space-y-4">
                {collections.map((collection) => (
                  ...
                  >
                      ...
                      {/* Gradient Overlay */}
                      <div className={`absolute inset-0 bg-gradient-to-r ${collection.gradient} to-transparent`} />
                      
                      {/* Content */}
                      <div className="absolute inset-0 flex flex-col justify-center p-6 md:p-8">
                          <span className={`text-${collection.color}-400 font-bold text-xs uppercase tracking-widest mb-2`}>
                            {collection.subtitle}
                          </span>
